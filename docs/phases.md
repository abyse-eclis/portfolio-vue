...
- [ ] Test: `supabase.from('profiles').select('*')` จาก browser ได้ data ไม่ error
...
- [ ] Test: เปิด `/` เห็น layout เปล่า + toggle ภาษา cookie persist
...
- [ ] Audit: Lighthouse Performance ≥ 90 mobile
...
- [ ] Edge Function `submit-contact` (email + rate limit) *(optional สำหรับ production)*
...
- [ ] `sitemap.xml` *(generate ด้วย script หรือ Netlify plugin)*
...
- [x] ESLint ผ่าน
- [ ] Vitest unit test: `tField()`, `useFormatPrice`, mappers *(optional)*
